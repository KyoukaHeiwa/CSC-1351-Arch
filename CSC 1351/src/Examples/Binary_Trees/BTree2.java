...
/**
 * This is a different style of writing the recursive methods
 * on the BTree. We only put methods on the BTree class, not
 * on the Node2.
 */
class Node2 {
  char c;
...
public class BTree2 {
  ...
  void add(Node2 n,char c) {
    if(c < n.c) {
        if(n.left == null) n.left = new Node2(c);
        else add(n.left,c);
    }
    if(n.c < c) {
        if(n.right == null) n.right = new Node2(c);
        else add(n.right,c);
    }
  }
   ...
   * is contained within the BTree2.
   */
  public boolean contains(char c) {
    return contains(head,c);
  }
  boolean contains(Node2 node,char c) {
    if(node == null) return false;
    if(node.c == c) return true;
    if(c < node.c && contains(node.left,c)) return true;
    if(node.c < c && contains(node.right,c)) return true;
    return false;
  }
